...
public class ColorDisplay extends JComponent {
	...
	public ColorDisplay(int verticalPages, int horizontalpages, int background, int grid, int gridStroke,
			int sideSize) {
	...
	public void setDisplay(int[][] colors, int verticalPage, int horizontalPage) {
		if (colors.length == PAGE_SIZE && colors[0].length == PAGE_SIZE && verticalPage >= 0
				&& verticalPage < verticalPages && horizontalPage >= 0 && horizontalPage < horizontalPages) {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int row = 0; row < systemPaint.length; row++) {
			for (int col = 0; col < systemPaint[row].length; col++) {
				...
				dBufferG.fillRect(offsetX + gridStroke + col * (side + gridStroke),
						offsetY + gridStroke + row * (side + gridStroke), side, side);
